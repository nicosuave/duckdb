...
static std::string linenoise_ascii_lower(const std::string &input) {
	std::string result;
	result.reserve(input.size());
	for (auto c : input) {
		auto uc = static_cast<unsigned char>(c);
		if (uc >= 'A' && uc <= 'Z') {
			result.push_back(static_cast<char>(uc - 'A' + 'a'));
		} else {
...
static std::string linenoise_ascii_lower(const char *input) {
	std::string result;
	if (!input) {
		return result;
	}
...
void Linenoise::PerformSearch() {
	...
	auto lsearch = linenoise_ascii_lower(search_buf);
	for (size_t i = History::GetLength(); i > 0; i--) {
		size_t history_index = i - 1;
		auto lhistory = linenoise_ascii_lower(History::GetEntry(history_index));
...
int Linenoise::Edit() {
	...
	while (true) {
		...
		case CTRL_J:
		case ENTER: { /* enter */
#if defined(LINENOISE_EDITOR) && !defined(DUCKDB_RUST_CLI_DISABLE_LINENOISE_EDITOR)
			if (len > 0) {
